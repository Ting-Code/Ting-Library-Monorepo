...
class MeshReflectorMaterial extends kokomi.Component {
  ...
  constructor(
    ...
    const { resolution = 256, ignoreObjects = [] } = config
  ...
  update(): void {
    ...
    this.mipmapper.update(this.mirrorFBO.texture, this.mipmapFBO.rt, this.base.renderer)
  }
  beforeRender() {
    ...
    d.applyQuaternion(this.base.camera.getWorldQuaternion(new THREE.Quaternion()))
    d.add(o)
    ...
    this._camera.applyQuaternion(this.base.camera.getWorldQuaternion(new THREE.Quaternion()))
